		      ...
		      void *srs_params)
{
	...
	pr_info("SRS - %s: Command was sent now check Q6 - port id = %d, size %d, module id %x, param id %x,mem_map_handle is %x.\n",
			__func__, adm_params->hdr.dest_port,
			adm_params->payload_size, module_id, param_id,adm_params->mem_map_handle);
