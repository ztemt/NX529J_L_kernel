...
#define NUBIA_WAIT_FOR_I2C 20
#define WAKEUP_GESTURE true
#define NUBIA_SYNAPTICS_PALM_SUPPORT
#if defined(NUBIA_SYNAPTICS_PALM_SUPPORT)
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
			wake_lock_timeout(&rmi4_data->gesture_wakelock,msecs_to_jiffies(500));
			dev_info(rmi4_data->pdev->dev.parent,"Double click.\n");
		...
		bool report)
{
	 ...
	 */
	for(retry = 0;retry < NUBIA_WAIT_FOR_I2C;retry++)
	{
		if(rmi4_data->waked_from_sleep == true)
			break;
		msleep(10);
	}
	if(retry >= NUBIA_WAIT_FOR_I2C)
	{
		dev_err(rmi4_data->pdev->dev.parent,"irq wake up timeout.\n");
		return;
	}
	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f01_data_base_addr,
		...
		bool get)
{
	...
	if ((bdata->pwr_reg_name != NULL) && (*bdata->pwr_reg_name != 0)) {
		...
		if (regulator_count_voltages(rmi4_data->pwr_reg) > 0) {
			...
			if (retval) {
				dev_err(rmi4_data->pdev->dev.parent,
					"Regulator set failed pwr_reg retval=%d\n",
					retval);
				goto regulator_put;
			}
		}
	...
	if ((bdata->bus_reg_name != NULL) && (*bdata->bus_reg_name != 0)) {
		...
		if (regulator_count_voltages(rmi4_data->bus_reg) > 0) {
			...
			if (retval) {
				dev_err(rmi4_data->pdev->dev.parent,
					"Regulator set failed bus_reg retval=%d\n",
					retval);
				goto regulator_put;
			}
		}
...
static void synaptics_set_pinctrl(const struct synaptics_dsx_board_data *bdata, bool active)
{
	int ret = 0;
	if(!bdata->pinctrl_info.pinctrl || !bdata->pinctrl_info.pin_active || !bdata->pinctrl_info.pin_suspend) {
		printk("%s : pinctrl is invalid, skip.\n",__func__);
		return;
	}
	if(active) {
		ret = pinctrl_select_state(bdata->pinctrl_info.pinctrl,bdata->pinctrl_info.pin_active);
	} else {
		ret = pinctrl_select_state(bdata->pinctrl_info.pinctrl,bdata->pinctrl_info.pin_suspend);
	}
	printk("%s : set pinctrl to [%s], ret = %d.\n",__func__,active ? "active" : "suspend",ret);
}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	rmi4_data->irq_enabled = false;
	rmi4_data->fingers_on_2d = false;
	rmi4_data->waked_from_sleep = true;
	...
	wake_lock_init(&rmi4_data->gesture_wakelock,WAKE_LOCK_SUSPEND,"synaptics-gesture-wakelock");

	platform_set_drvdata(pdev, rmi4_data);

	vir_button_map = bdata->vir_button_map;

	synaptics_set_pinctrl(bdata,true); //irq/reset pinctrl
...
err_get_reg:
	wake_lock_destroy(&rmi4_data->gesture_wakelock);
	kfree(rmi4_data);
...
static int synaptics_rmi4_pm_suspend(struct device *dev)
{
	...
	rmi4_data->waked_from_sleep = false;
	pr_err("[TP]%s: synaptics  pm suspend !\n", __func__);
	return 0;
}
...
static int synaptics_rmi4_pm_resume(struct device *dev)
{
	...
	rmi4_data->waked_from_sleep = true;
	pr_err("[TP]%s: synaptics pm resume !\n", __func__);
	return 0;
}
...
//ztemt ningyaodong modify for not loading TP driver in poweroffcharge begin
#define NUBIA_POWEROFF_CHARGE_NO_TP
#if defined(NUBIA_POWEROFF_CHARGE_NO_TP)
#define TP_BOOT_MODE_LENGTH 10
static char nubia_boot_mode[TP_BOOT_MODE_LENGTH];
static bool nubia_is_poweroffcharge(void)
{
	printk("[TP]boot_mode = %s.\n",nubia_boot_mode);
	if(strcmp(nubia_boot_mode,"charger") == 0)
		return true;
	else
		return false;
}
static int nubia_get_bootmode(char *boot_mode)
{
	//strncpy(nubia_boot_mode,saved_command_line,256);
	strncpy(nubia_boot_mode,boot_mode,TP_BOOT_MODE_LENGTH);
	nubia_boot_mode[TP_BOOT_MODE_LENGTH-1] = '\0';
	return 0;
}
__setup("androidboot.mode=",nubia_get_bootmode);
#endif
//ztemt ningyaodong modify for not loading TP driver in poweroffcharge end
...
static int __init synaptics_rmi4_init(void)
{
	...
	//ztemt ningyaodong modify for not loading TP driver in poweroffcharge begin
	#if defined(NUBIA_POWEROFF_CHARGE_NO_TP)
	if(nubia_is_poweroffcharge() == true)
		return 0;
	#endif
	//ztemt ningyaodong modify for not loading TP driver in poweroffcharge end
