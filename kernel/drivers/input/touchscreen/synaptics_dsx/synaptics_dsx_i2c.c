...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	bdata->pinctrl_info.pinctrl = devm_pinctrl_get(dev);
	if(IS_ERR_OR_NULL(bdata->pinctrl_info.pinctrl)) {
		pr_err("[TP]%s : synaptics get pinctrl info error.\n",__func__);
	} else {
		bdata->pinctrl_info.pin_active = pinctrl_lookup_state(bdata->pinctrl_info.pinctrl,"synaptics_int_active");
		if(IS_ERR_OR_NULL(bdata->pinctrl_info.pin_active)) {
			pr_err("[TP]%s : synaptics get pin_active info error.\n",__func__);
		}
		bdata->pinctrl_info.pin_suspend = pinctrl_lookup_state(bdata->pinctrl_info.pinctrl,"synaptics_int_suspend");
		if(IS_ERR_OR_NULL(bdata->pinctrl_info.pin_suspend)) {
