...
#define CYTTSP568_DEVICE_ACCESS_NAME "cyttsp5_device_access"
#define CYTTSP568_INPUT_ELEM_SZ (sizeof("0xHH") + 1)
...
#define CYTTSP568_TTHE_TUNER_GET_PANEL_DATA_FILE_NAME "get_panel_data"
		...
		size_t buf_size, u8 *ic_buf, size_t ic_buf_size)
{
	const char *pbuf = buf;
	unsigned long value;
	char scan_buf[CYTTSP568_INPUT_ELEM_SZ];
	u32 i = 0;
	u32 j;
	int last = 0;
	int ret;
	...
	while (pbuf <= (buf + buf_size)) {
		...
		memset(scan_buf, 0, CYTTSP568_INPUT_ELEM_SZ);
		if ((last == ',') && (*pbuf == ',')) {
			dev_err(dev, "%s: %s \",,\" not allowed.\n", __func__,
					"Invalid data format.");
			return -EINVAL;
		}
		for (j = 0; j < (CYTTSP568_INPUT_ELEM_SZ - 1)
				&& (pbuf < (buf + buf_size))
				&& (*pbuf != ' ')
				&& (*pbuf != ','); j++) {
			last = *pbuf;
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	dad->tthe_get_panel_data_debugfs = debugfs_create_file(
			CYTTSP568_TTHE_TUNER_GET_PANEL_DATA_FILE_NAME,
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP568_DEVICE_ACCESS_API
