...
#define TOUCHPANEL_IC_INFO

#ifdef TOUCHPANEL_IC_INFO
#include <linux/proc_fs.h>
#include <linux/seq_file.h>
#include <linux/ctype.h>
u32  revision_ctrl_num;
u32  fw_config_num;
static char *tp_name;
#endif
...
#ifdef CYPRESS_GLOBAL_CORE_DATA
struct cyttsp5_core_data g_core_data;
struct cyttsp5_core_data *cyttsp5_get_global_core_data(struct device *dev)
{
	return &g_core_data;
}
EXPORT_SYMBOL_GPL(cyttsp5_get_global_core_data);
#endif
...
{
	#ifdef CYPRESS_GLOBAL_CORE_DATA
	struct cyttsp5_core_data *cd = cyttsp5_get_global_core_data(dev);
	#else
...
static int setup_report_descriptor(struct cyttsp5_core_data *cd)
{
	...
	dev_vdbg(cd->dev, "Features: easywake:%d noise_metric:%d tracking_heatmap:%d sensor_data:%d\n",
...
{
	#ifdef CYPRESS_GLOBAL_CORE_DATA
	struct cyttsp5_core_data *cd = cyttsp5_get_global_core_data(NULL);
	#else
	struct cyttsp5_core_data *cd =
...
static int cyttsp5_read_input(struct cyttsp5_core_data *cd)
{
	...
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		if (cd->sleep_state == SS_SLEEP_ON) {
			...
			if (IS_TMO(t))
			{
				printk("cyttsp : int wait timeout.\n");
				cyttsp5_queue_startup(cd);
			}
			goto read;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf,
		...
		"%s: 0x%02X\n"
		"%s: 0x%02X\n",
		"Cyttsp5 Firmware Version in TP IC ",cydata->fw_ver_conf,
...
static ssize_t name_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf) {
   ...
   return sprintf(buf,"REVISION_CTRL_NUM 0x%x, FW_CONFIG_NUM %d, TP_NAME %s\n", revision_ctrl_num, fw_config_num, tp_name);
}
...
static void device_info_show(void)
{
        ...
        properties_kobj = kobject_create_and_add("tp_info", NULL);
        if (properties_kobj)
                ret = sysfs_create_group(properties_kobj,
                                &properties_attr_group);
        if (!properties_kobj || ret)
                printk(KERN_ERR "failed to create tp_info\n");
}

static void touchpanel_ic_property_create(void)
{
    printk(KERN_ERR "%s:JackChen ---touchpanel--- \n", __func__);

    device_info_show();
}

static void touchpanel_ic_save_info(struct device *dev)
{
	#ifdef CYPRESS_GLOBAL_CORE_DATA
	struct cyttsp5_core_data *cd = cyttsp5_get_global_core_data(dev);
	#else
	...
	struct cyttsp5_cydata *cydata = &cd->sysinfo.cydata;
        ...
        tp_name = "cyttsp";
	printk(KERN_ERR "%s: tp_name:%s\n", __func__, tp_name);

        revision_ctrl_num = cydata->revctrl;
        printk(KERN_ERR "%s: revision_ctrl_num:0x%0x\n", __func__, revision_ctrl_num);

        fw_config_num = cydata->fw_ver_conf;
        printk(KERN_ERR "%s: fw_config_num:%d\n", __func__, fw_config_num);     
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	#ifndef CYPRESS_GLOBAL_CORE_DATA
