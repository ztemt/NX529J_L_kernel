...
/* ztemt add for fw version check begin*/
#define FW_VER_NEW 0x2802
#define STD_FW_VER_H_MASK 0xff00
#define STD_FW_VER_H 0x2800
static const u16 compatible_fw_ver[] =
{
	0x0000,
	0x0100,
};
#define FW_FILE_NAME "cyttsp5_fw.bin"
/* ztemt add for fw version check end */
...
static int cyttsp5_check_firmware_version(struct device *dev,
		u16 fw_ver_conf_new, u32 fw_revctrl_new)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);

	//ztemt add for fw upgrade begin
	u32 fw_ver_img = ld->si->cydata.fw_ver_conf;
	u8 size = sizeof(compatible_fw_ver)/sizeof(u16);
	int i = 0;
	printk("curren img ver : 0x%04x ,new fw ver : 0x%04x.\n",fw_ver_img,fw_ver_conf_new);

	if((fw_ver_conf_new & STD_FW_VER_H_MASK) != STD_FW_VER_H)
	{
		dev_err(dev,"%s: unknown fw ver:0x%04x ,will Not upgrade.\n",__func__,fw_ver_img);
		return -1;
	}
	//Upgrade FW when the old ver is 0x00** or 0x01**.
	for(i = 0;i < size;i++)
	{
		if((fw_ver_img & STD_FW_VER_H_MASK) == compatible_fw_ver[i])
		{
			dev_err(dev,"%s : img ver:0x%04x is a compatible old img,should upgrade.\n",
				__func__,fw_ver_img);
			return 1;
		}
	}
	//Do not upgrade when the old ver is unknown
	if((fw_ver_img & STD_FW_VER_H_MASK) != STD_FW_VER_H)
	{
		dev_err(dev,"%s: unknown img ver:0x%04x ,will Not upgrade.\n",__func__,fw_ver_img);
		return -1;
	}
        //Upgrade when the FW ver is newer than that in IC
	if(fw_ver_img < fw_ver_conf_new)
	{
		dev_err(dev,"%s : newer img found, should upgrade.\n",__func__);
		return 1;
	}
	//ztemt add for fw upgrade end
	return -1;
}
		...
		const struct firmware *fw)
{
	...
	upgrade = cyttsp5_check_firmware_version(dev, fw_ver_conf_new,
			fw_revctrl_new);
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
	...
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			FW_FILE_NAME, dev, GFP_KERNEL, dev,
