...
//ZTEMT ningyaodong modify for open short test begin
enum TEST_CASE
{
	FULL_CMCP_TEST=0,
	CM_PANEL_TEST,
...
void cyttsp5_config_test_type(struct device *dev,struct cyttsp5_device_access_data *dad)
{
	pm_runtime_get_sync(dev);
	mutex_lock(&dad->sysfs_lock);
	
	dad->cmcp_test_items = FULL_CMCP_TEST;
	dad->cmcp_range_check = RANGE_FULL;
	dad->cmcp_force_calibrate = CALI_IN_TEST;
	
	mutex_unlock(&dad->sysfs_lock);
	pm_runtime_put(dev);
}

int cyttsp5_open_autodetect(struct device *dev,struct cyttsp5_device_access_data *dad)
{
	struct cmcp_data *cmcp_info = dad->cmcp_info;
	struct result *result = dad->result;
	struct configuration *configuration = dad->configs;
...
cmcp_ready:
	...
	mutex_unlock(&dad->sysfs_lock);
exit:
	return dad->result->test_summary ? TEST_PASS:TEST_FAIL;
}
static ssize_t cyttsp5_cmcp_autodetect_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	/*struct cmcp_data *cmcp_info = dad->cmcp_info;
	struct result *result = dad->result;
	...
	cyttsp5_config_test_type(dev,dad);
	results = cyttsp5_open_autodetect(dev,dad);

	return snprintf(buf, PAGE_SIZE,"%d\n", results);
}
static DEVICE_ATTR(ic_detect, 0444,cyttsp5_cmcp_autodetect_show,NULL);
//ztemt ningyaodong modify for open short test end
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	if(rc){
		dev_err(dev, "%s: Error, could not create cmcp_autodetect_results\n",
				__func__);
	}
